...
function ToggleRow(
	...
	return (
		<div className="flex flex-row items-start justify-between gap-3 rounded px-2 py-1.5 hover:bg-zinc-800/60">
			<div className="flex min-w-0 flex-col">
				<Label htmlFor={id} className="cursor-pointer text-xs text-zinc-200">{label}</Label>
		...
		<div className="flex flex-col gap-1 px-2 pb-2">
			...
			>
				<ComboboxTrigger
					className={cn([
						...
						"border border-zinc-700/70 bg-zinc-800/60 text-xs text-zinc-200 hover:bg-zinc-800"
					])}
				>
			...
			{selected && (
				<button
					...
					className="self-start text-[10px] text-zinc-500 hover:text-zinc-200"
				>
					Show all roles
				</button>
			)}
...
function Group({title, children}: { title: string; children: React.ReactNode }) {
	return (
		<div className="flex flex-col border-b border-zinc-800 py-1 last:border-b-0">
			<span className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-zinc-500">{title}</span>
			{children}
		</div>
	);
}
...
export function MapSettingsPanel() {
	...
	if (s.panelCollapsed) {
		return (
			<button
				...
				title="Map settings"
				className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-md border border-zinc-700/70 bg-zinc-900/90 text-zinc-300 hover:bg-zinc-800 hover:text-zinc-100"
			>
				<Settings2 size={16} />
			</button>
		);
	...
	return (
		<div className="absolute right-3 top-3 z-10 w-60 overflow-hidden rounded-md border border-zinc-700/70 bg-zinc-900/95 shadow-lg">
