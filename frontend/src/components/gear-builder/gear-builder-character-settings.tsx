...
export function GearBuilderCharacterSettings() {
	const {gearMastery : mastery, level} = useSnapshot(gearBuilderStore);
	...
	const save = (skill: LifeSkillType, patch: Partial<MasteryData>) => {
		...
		gearBuilderStore.gearMastery = next;

		updateDebounced(next);
	};

	const setLevelDebounced = useDebounce((l: number) => {
		void SetLevel(l);
	}, 300);

	const saveLevel = (l: number) => {
		const next             = Math.max(1, l);
		gearBuilderStore.level = next;
