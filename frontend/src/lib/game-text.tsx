...
import type {MaybeNullable} from "@/types.ts";

export type TextSegment = { text: string; color?: string };

const TOKEN_RE = /<PAColor0x([0-9a-fA-F]{6,8})>|<PAOldColor>/g;
const STRIP_RE = /<PAColor0x[0-9a-fA-F]{6,8}>|<PAOldColor>/g;
...
export function parseGameText(input: MaybeNullable<string>): TextSegment[] {
	if (!input)
		return [];
	...
	const push = (text: string) => {
		if (text)
			segments.push({text, color : stack[stack.length - 1]});
	};

	TOKEN_RE.lastIndex = 0;
	while ((m = TOKEN_RE.exec(input)) !== null) {
		push(input.slice(last, m.index));
		// m[1] is an optional capture group — undefined at runtime when the alternative matched.
		if (m[1] !== undefined) {
			// <PAColor0x…>
...
// eslint-disable-next-line react-refresh/only-export-components
export function plainGameText(input: MaybeNullable<string>): string {
	return (input ?? "").replace(STRIP_RE, "");
}

export function GameText({text, className, textClassName}: { text: MaybeNullable<string>, className?: string, textClassName?: string }) {
